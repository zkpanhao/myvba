Sub MergeBooks()
    ...
    Dim ActiveB As Workbook, Headline As Byte, Namess As String, Cell As Range
...
'另一种写法结束
    ...
    Headline = Application.InputBox("输入待合并的表的标题行数:", "输入框", 1, , , , , 1)    '获取用户输入的标题行数
    If Headline < 0 Then
        Exit Sub                                               '小于0退出程序
    ElseIf Headline = False Then                               '选择取消退出程序
        Exit Sub
    End If
    On Error GoTo 0
    Range("A" & Headline & ":B" & Headline) = Array("工作簿", "工作表")    '留两列放来源表名
